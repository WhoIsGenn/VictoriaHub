...
--------------------------------------------------
-- ========== TAB 3 : SKIN ANIMATION ==========
--------------------------------------------------

local AnimationSection = Tab3:Section({
...
local function SetupCharacter(char)
    ...
    Animator.AnimationPlayed:Connect(function(track)
        if BlockRodAnim and track ~= CurrentTrack then
            track:Stop()
        end
    end)
end
...
-- PLAY ANIMATION SAFE
local function PlayFishAnimation(animName)
    if not Animator then return end

    local animFolder = ReplicatedStorage:FindFirstChild("Modules")
        and ReplicatedStorage.Modules:FindFirstChild("Animations")
    ...
    CurrentTrack = Animator:LoadAnimation(animObject)
...
for name in pairs(AnimationList) do
    table.insert(AnimOptions, name)
end
table.sort(AnimOptions)
...
AnimationSection:Dropdown({
    ...
    Callback = function(selected)
        ActiveAnimName = AnimationList[selected]
    end
})
...
-- FISH CAUGHT EVENT
Net:RemoteEvent("FishCaught").OnClientEvent:Connect(function()
    if ActiveAnimName then
        PlayFishAnimation(ActiveAnimName)
    end
end)
...
local AutoTotemSection = Tab4:Section({
    Title = "Auto Totem",
    Icon = "zap",
    TextXAlignment = "Left",
    TextSize = 17,
...
local UniqueTotems = {}
for _, t in pairs(GetTotems()) do
    if not UniqueTotems[t.Id] then
        UniqueTotems[t.Id] = TotemNames[t.Id] or ("Totem ID " .. t.Id)
    end
end

local TotemOptions = {}
for _, name in pairs(UniqueTotems) do
    table.insert(TotemOptions, name)
end

AutoTotemSection:Dropdown({
    Title = "Select Totem",
    Values = TotemOptions,
    Callback = function(selected)
        for id, name in pairs(TotemNames) do
    ...
    Callback = function(state)
        ...
        task.spawn(function()
            while AutoTotemEnabled do
                for _, totem in pairs(GetTotems()) do
                    if totem.Id == SelectedTotemId then
                        Net:RemoteEvent("SpawnTotem"):FireServer(totem.UUID)
                        break
                    end
                end
