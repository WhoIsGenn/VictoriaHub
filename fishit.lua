...
local FavRarityList = {"Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic", "SECRET"}
...
local DestroyedObjects = {} -- Track destroyed objects
...
-- Cache function (with destroy tracking)
local function cache(o)
    ...
    elseif o:IsA("Decal") or o:IsA("Texture") or o:IsA("SurfaceAppearance") then
        Cache[o] = {
            Transparency = o:IsA("Decal") and o.Transparency or nil,
            Parent = o.Parent
        }
    elseif o:IsA("ParticleEmitter") or o:IsA("Trail") or o:IsA("Smoke") 
        or o:IsA("Fire") or o:IsA("Sparkles") or o:IsA("Beam") then
        Cache[o] = {
            Enabled = o.Enabled,
            Parent = o.Parent
        }
...
local function apply(o)
    -- Ignore player character and UI
    ...
    pcall(function()
        ...
        -- MeshPart optimization + DESTROY mesh
        elseif o:IsA("MeshPart") then
            o.RenderFidelity = Enum.RenderFidelity.Performance
            o.Material = Enum.Material.Plastic
            o.Reflectance = 0
            o.TextureID = ""
            o.MeshId = "" -- Remove mesh, jadi kotak
            o.Color = White
            o.CastShadow = false
            
        -- DESTROY all mesh types
        elseif o:IsA("SpecialMesh") or o:IsA("FileMesh") or o:IsA("CylinderMesh") 
            or o:IsA("BlockMesh") then
        ...
        -- DESTROY clothing
        elseif o:IsA("Clothing") or o:IsA("ShirtGraphic") or o:IsA("Pants") 
            or o:IsA("Shirt") or o:IsA("BaseWrap") or o:IsA("Accessory") then
            table.insert(DestroyedObjects, o)
            o:Destroy()
        ...
        elseif o:IsA("Model") then
            o.LevelOfDetail = Enum.ModelLevelOfDetail.StreamingMesh
            
        -- DESTROY explosions visual
...
-- Restore function (Won't restore destroyed objects)
...
graphic:Toggle({
    ...
    Callback = function(v)
        ...
        Lighting.ShadowSoftness = v and 0 or 0.2
        Lighting.Brightness = v and 2 or 1
        Lighting.OutdoorAmbient = v and Color3.fromRGB(128, 128, 128) or Color3.fromRGB(70, 70, 70)
        Lighting.Ambient = v and Color3.fromRGB(128, 128, 128) or Color3.fromRGB(0, 0, 0)
        Lighting.EnvironmentDiffuseScale = v and 0 or 1
        Lighting.EnvironmentSpecularScale = v and 0 or 1
        
        -- Technology setting (Hidden Property)
        pcall(function()
            if sethiddenproperty then
                sethiddenproperty(Lighting, "Technology", v and Enum.Technology.Compatibility or Enum.Technology.ShadowMap)
            end
        end)
        
        -- DESTROY all post-processing effects
        for _, e in ipairs(Lighting:GetChildren()) do
            if v then
                if e:IsA("BloomEffect") or e:IsA("SunRaysEffect") or e:IsA("BlurEffect")
                    ...
                    or e:IsA("Atmosphere") or e:IsA("Sky") or e:IsA("Clouds") then
                    e:Destroy()
                end
            end
        end
        ...
        pcall(function()
            ...
            if terrain then
                terrain.WaterWaveSize = v and 0 or 0.15
                terrain.WaterWaveSpeed = v and 0 or 10
                terrain.WaterReflectance = v and 0 or 1
                terrain.WaterTransparency = v and 0.5 or 0.3
                
                -- Decoration (Hidden Property)
                if sethiddenproperty then
        ...
        pcall(function()
            ...
            settings().Rendering.MeshPartDetailLevel = v and Enum.MeshPartDetailLevel.Level04 or Enum.MeshPartDetailLevel.DistanceBased
        end)
        
        -- === MATERIAL SERVICE (DESTROY custom materials) ===
        if v then
            pcall(function()
        ...
        -- === FPS CAP ===
        pcall(function()
            if setfpscap and v then
                setfpscap(999) -- Uncap FPS
            end
        end)

        -- === PROCESS ALL WORKSPACE OBJECTS ===
        DestroyedObjects = {} -- Reset tracking
