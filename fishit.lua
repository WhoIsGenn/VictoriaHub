...
-- ===============================
-- SKIN ANIMATION (FULL FIX)
-- ===============================

local animation = Tab3:Section({ 
...
local function PlayFishAnim(animName)
    ...
    local animFolder = _G.RS:FindFirstChild("Modules")
        and _G.RS.Modules:FindFirstChild("Animations")

    if not animFolder then
        warn("[SkinAnimation] Animations folder not found")
        return
    end

    local animObj = animFolder:FindFirstChild(animName)
    if not animObj then
        warn("[SkinAnimation] Animation not found:", animName)
        return
    end
    ...
    _G.CurrentTrack = _G.Animator:LoadAnimation(animObj)
...
animation:Button({
    ...
    Callback = function()
        if not _G.SelectedAnim then return end
...
-- ===============================
-- FISH CAUGHT EVENT (FIXED)
-- ===============================

task.defer(function()
    -- tunggu Net siap TANPA nge-freeze UI
    local net
    for i = 1, 200 do
        net = _G.Net
    ...
    local fishEvent
    for i = 1, 200 do
        fishEvent = net:FindFirstChild("RE/FishCaught")
        if fishEvent then break end
        task.wait(0.05)
    end

    if not fishEvent then
        warn("[SkinAnimation] FishCaught event not found")
        return
    end
