...
WindUI:SetTheme("black and white")
...
if executorName:lower():find("flux") then
    executorColor = Color3.fromHex("#2d2e2e")     -- Fluxus
elseif executorName:lower():find("delta") then
    executorColor = Color3.fromHex("#2d2e2e")     -- Delta
...
elseif executorName:lower():find("seliware") then
    executorColor = Color3.fromHex("#2d2e2e")     -- Seliware
...
event:Toggle({
    ...
    Desc = "You Have to Take the Quest First",
    Value = false,
    Callback = function(state)
...
local RS = game:GetService("ReplicatedStorage")
local Net = RS.Packages._Index["sleitnick_net@0.2.0"].net
local Claim = Net["RE/ClaimPirateChest"]
...
blantant:Button({
    ...
    Callback = function()
        ...
        if success then
            WindUI:Notify({
                Title = "Recovery Fishing",
                Content = "Fishing successfully recovered",
                Duration = 3,
...
_G.CreateFloatingIcon = function()
    ...
    _G.IconImage.Image = "rbxassetid://71947103252559"
