...
-- ======================================================
-- PATCH FEATURES (SAFE FOR WINDUI)
-- ======================================================
...
task.defer(function()
    ...
    if not Net then
        warn("[VictoriaHub] Net not loaded, features disabled")
        return
    end
    ...
    for _ = 1, 200 do
        ...
        if DataReplion then break end
        task.wait(0.05)
    end

    if not DataReplion then
        warn("[VictoriaHub] DataReplion timeout")
        return
    end
    ...
    local AnimSection = Tab3:Section({
    ...
    local function SetupChar(char)
        ...
        Animator.AnimationPlayed:Connect(function(track)
            if BlockAnim and track ~= CurrentTrack then
                track:Stop()
            end
        end)
    end
    ...
    local AnimList = {}
    for k in pairs(AnimMap) do table.insert(AnimList, k) end
    table.sort(AnimList)
    ...
    local function GetInventoryTotems()
    ...
    local Seen = {}
    local Options = {}
    for _, t in pairs(GetInventoryTotems()) do
        if not Seen[t.Id] then
            Seen[t.Id] = true
            table.insert(Options, TotemNames[t.Id] or ("Totem "..t.Id))
        end
    end
    ...
    TotemSection:Input({
        ...
        Callback = function(v)
            local n = tonumber(v)
            if n and n > 0 then DelayMin = n end
        end
    })
                ...
                while Enabled do
                    for _, t in pairs(GetInventoryTotems()) do
                        if t.Id == SelectedId then
                            Net:RemoteEvent("SpawnTotem"):FireServer(t.UUID)
                            break
                        end
                    end
    ...
    print("[VictoriaHub] Features loaded safely")
