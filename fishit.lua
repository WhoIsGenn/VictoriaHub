...
-- ========== SIMPLE RECOVERY FISHING ==========
local function doRecoveryFishing()
    -- STEP 1: Stop blantant fishing
    c.d = false
    
    -- Cancel running tasks
    if m then 
        local success = pcall(task.cancel, m)
        m = nil
    end
    if n then 
        local success = pcall(task.cancel, n)
        n = nil
    end
    
    -- STEP 2: Cancel fishing
    pcall(function()
        if l then
            l:InvokeServer()
        end
