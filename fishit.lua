...
-- ======================================================
-- BLATANT V3 CORE (AUTHOR VERSION)
-- SAME FUNCTION, BETTER STABILITY
-- ======================================================
...
v3 = {
    ...
    -- 🔑 MAIN SETTINGS
    ReelDelay     = 1.45,  -- loop delay
    CompleteDelay = 0.23,  -- finish delay
}
...
-- ======================================================
-- CORE CYCLE (SAMA KAYA BLATANT LO)
-- ======================================================
local function V3_FishingCycle()
    task.spawn(function()
        -- cancel
        while v3.Enabled and not invoke(RF_Cancel) do
            task.wait(0.05)
        end

        -- charge
        while v3.Enabled and not invoke(RF_Charge, 9e9) do
            task.wait(0.05)
        end

        -- start minigame
        invoke(RF_Start, -139.63, 0.996)
    end)

    task.spawn(function()
        task.wait(v3.CompleteDelay)
        if v3.Enabled then
            fire(RE_Done)
        end
    end)
...
-- MAIN LOOP
-- ======================================================
local function V3_Loop()
    v3_equip = task.spawn(function()
        while v3.Enabled do
            fire(RE_Equip, 1)
            task.wait(1.6)
        end
...
-- TOGGLE FUNCTION
-- ======================================================
function ToggleV3(state)
    v3.Enabled = state

    if state then
        if v3_main then task.cancel(v3_main) end
        if v3_equip then task.cancel(v3_equip) end
        v3_main = task.spawn(V3_Loop)
    else
        if v3_main then task.cancel(v3_main) end
        if v3_equip then task.cancel(v3_equip) end
        v3_main, v3_equip = nil, nil
        invoke(RF_Cancel)
    end
end
...
-- SECTION 3: BLATANT V3 (SIMPLIFIED)
blantantV3Section = Tab3:Section({
    Title = "Blatant V3",
    Icon = "fish",
    TextTransparency = 0.05,
    TextXAlignment = "Left",
...
-- REEL / LOOP DELAY
blantantV3Section:Input({
    Title = "Reel Delay",
    Placeholder = "1.45",
    Default = tostring(v3.LoopDelay),
    Callback = function(input)
        local num = tonumber(input)
...
blantantV3Section:Input({
    Title = "Complete Delay",
    Placeholder = "0.23",
    Default = tostring(v3.CompleteDelay),
    Callback = function(input)
        local num = tonumber(input)
        if num and num > 0.1 then
            v3.CompleteDelay = num
        end
    end
})
