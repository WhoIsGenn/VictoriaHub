...
-- ===============================
-- AUTO TOTEM (FINAL FIX)
-- ===============================

task.spawn(function()

    -- WAIT TAB4 GLOBAL
    while not _G.WindUI_Tab4 do
        task.wait()
    end

    local Tab4 = _G.WindUI_Tab4
    ...
    local function getTotems()
        ...
        if inv and inv.Totems then
            for _, t in pairs(inv.Totems) do
                if not seen[t.Id] then
                    seen[t.Id] = true
                    table.insert(list, {Name = getTotemName(t.Id), Id = t.Id})
                end
            end
        end
        return list
    ...
    local types = getTotems()
    local options = {}

    for _, t in ipairs(types) do
        table.insert(options, t.Name)
    end
        ...
        Callback = function(on)
            ...
            task.spawn(function()
                while _G.AutoTotemEnabled do
                    local inv = DataReplion:Get("Inventory")
                    if inv and inv.Totems then
                        local spawn = Net:RemoteEvent("SpawnTotem")
                        for _, t in pairs(inv.Totems) do
                            if t.Id == _G.SelectedTotemId then
                                spawn:FireServer(t.UUID)
                                break
                            end
                        end
                    end
    ...
    print("[AutoTotem] Section loaded")
