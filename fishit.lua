...
-- ======================================================
-- SKIN ANIMATION (FINAL SAFE VERSION)
-- ======================================================

task.spawn(function()

    -- ===============================
    -- WAIT FOR WINDUI TAB3
    -- ===============================
    local Tab3Ref
    for i = 1, 300 do
        if rawget(getfenv(), "Tab3") then
            Tab3Ref = Tab3
            break
        end
        task.wait(0.05)
    ...
    if not Tab3Ref then
        warn("[SkinAnimation] Tab3 not found, aborting")
        return
    end
    ...
    -- CREATE SECTION (SAFE)
    -- ===============================
    local animation = Tab3Ref:Section({ 
    ...
    if not Net then
        warn("[SkinAnimation] Net not found")
        return
    end
    ...
    local function PlayFishAnim(animName)
        ...
        local animFolder = RS:FindFirstChild("Modules")
            and RS.Modules:FindFirstChild("Animations")
    ...
    local animNames = {}
    for k in pairs(AnimationList) do table.insert(animNames, k) end
    ...
    task.spawn(function()
        local ev
        for i = 1, 200 do
            ev = Net:FindFirstChild("RE/FishCaught")
        ...
        ev.OnClientEvent:Connect(function()
            if ActiveAnim then
                PlayFishAnim(ActiveAnim)
            end
        end)
    end)

    print("[SkinAnimation] Loaded safely")
