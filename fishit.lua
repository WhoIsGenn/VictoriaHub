...
local Window = WindUI:CreateWindow({
    ...
    HasOutline = true,                                                          
    OpenButton = {
        Title = "Victoria Hub", -- can be changed
        Icon = "rbxassetid://134034549147826",
...
-- ========== BLATANT V3 CONFIG (VERSI CLAUDE) ==========
local V3Config = {
    IsRunning = false,
    ReelDelay = 1.55,
    CompleteDelay = 0.22
...
local Remotes = {}
pcall(function()
    Remotes.Charge = netFolder:WaitForChild("RF/ChargeFishingRod")
    Remotes.StartMinigame = netFolder:WaitForChild("RF/RequestFishingMinigameStarted")
    Remotes.Complete = netFolder:WaitForChild("RE/FishingCompleted")
    Remotes.Equip = netFolder:WaitForChild("RE/EquipToolFromHotbar")
    Remotes.Cancel = netFolder:WaitForChild("RF/CancelFishingInputs")
end)
...
local function ExecuteCast()
    task.spawn(function()
        pcall(function()
            ...
            local cancelSuccess = Remotes.Cancel:InvokeServer()
            if not cancelSuccess then
                while not cancelSuccess do
                    cancelSuccess = Remotes.Cancel:InvokeServer()
            ...
            local chargeSuccess = Remotes.Charge:InvokeServer(tick())
            if not chargeSuccess then
                while not chargeSuccess do
                    chargeSuccess = Remotes.Charge:InvokeServer(tick())
            ...
            -- Start minigame
            Remotes.StartMinigame:InvokeServer(-139.63, 0.996)
        end)
    end)
    ...
    task.spawn(function()
        ...
        if V3Config.IsRunning then
            pcall(Remotes.Complete.FireServer, Remotes.Complete)
        end
    end)
end
...
local function EquipLoop()
    while V3Config.IsRunning do
        pcall(Remotes.Equip.FireServer, Remotes.Equip, 1)
        task.wait(1.5)
    end
end
...
local function ToggleV3(enabled)
    ...
    if enabled then
        ...
        -- Start new threads
        MainThread = task.spawn(FishingLoop)
        ...
        -- Cancel fishing
        pcall(Remotes.Cancel.InvokeServer, Remotes.Cancel)
    end
end
...
blatantV3Section:Toggle({
    Title = "Blatant V3",
    Value = V3Config.IsRunning,
    Callback = function(value)
        ToggleV3(value)
    end
})

blatantV3Section:Input({
    Title = "Reel Delay",
    Placeholder = "0.7",
...
_G.CreateFloatingIcon = function()
    _G.existingGui = _G.PlayerGui:FindFirstChild("CustomFloatingIcon_RockHub")
    if _G.existingGui then _G.existingGui:Destroy() end

    _G.FloatingIconGui = Instance.new("ScreenGui")
    _G.FloatingIconGui.Name = "CustomFloatingIcon_RockHub"
    _G.FloatingIconGui.DisplayOrder = 999
    _G.FloatingIconGui.ResetOnSpawn = false

    _G.FloatingFrame = Instance.new("Frame")
    _G.FloatingFrame.Name = "FloatingFrame"
    _G.FloatingFrame.Position = UDim2.new(0, 50, 0.4, 0)
    _G.FloatingFrame.Size = UDim2.fromOffset(65, 65)
    _G.FloatingFrame.AnchorPoint = Vector2.new(0.5, 0.5)
    _G.FloatingFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
    _G.FloatingFrame.BackgroundTransparency = 0
    _G.FloatingFrame.BorderSizePixel = 0
    _G.FloatingFrame.Parent = _G.FloatingIconGui

    _G.FrameStroke = Instance.new("UIStroke")
    _G.FrameStroke.Color = Color3.fromHex("#80ccf2")
    _G.FrameStroke.Thickness = 2
    _G.FrameStroke.Transparency = 0
    _G.FrameStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    _G.FrameStroke.Parent = _G.FloatingFrame

    _G.FrameCorner = Instance.new("UICorner")
    _G.FrameCorner.CornerRadius = UDim.new(0, 12)
    _G.FrameCorner.Parent = _G.FloatingFrame

    _G.IconImage = Instance.new("ImageLabel")
    _G.IconImage.Name = "Icon"
    _G.IconImage.Image = "rbxassetid://134034549147826"
    _G.IconImage.BackgroundTransparency = 1
    _G.IconImage.Size = UDim2.new(1, -4, 1, -4)
    _G.IconImage.Position = UDim2.new(0.5, 0, 0.5, 0)
    _G.IconImage.AnchorPoint = Vector2.new(0.5, 0.5)
    _G.IconImage.Parent = _G.FloatingFrame

    _G.ImageCorner = Instance.new("UICorner")
    _G.ImageCorner.CornerRadius = UDim.new(0, 10)
    _G.ImageCorner.Parent = _G.IconImage

    _G.FloatingIconGui.Parent = _G.PlayerGui
    return _G.FloatingIconGui, _G.FloatingFrame
end
...
_G.SetupFloatingIcon = function(FloatingIconGui, FloatingFrame)
    ...
    _G.update = function(input)
        _G.delta = input.Position - _G.dragStart
        FloatingFrame.Position = UDim2.new(
            _G.startPos.X.Scale,
            _G.startPos.X.Offset + _G.delta.X,
            _G.startPos.Y.Scale,
            _G.startPos.Y.Offset + _G.delta.Y
        )
    end
    ...
    FloatingFrame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            _G.connection = input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    _G.dragging = false
                    _G.connection:Disconnect()
                    if not _G.didMove then
            ...
            _G.moveConnection = input.Changed:Connect(function()
                if _G.dragging and (input.Position - _G.dragStart).Magnitude > 5 then
                    _G.didMove = true
...
_G.InitializeIcon = function()
    ...
    _G.gui, _G.frame = _G.CreateFloatingIcon()
    if _G.gui and _G.frame then
        _G.SetupFloatingIcon(_G.gui, _G.frame)
    end
end
