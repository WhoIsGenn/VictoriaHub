...
task.spawn(function()
    while task.wait(1) do
        ...
        -- Start fishing with LEGIT mode
        if not _G.QuestState.Fishing then
            _G.AutoFishing = true
            -- Use LEGIT fishing mode
            task.spawn(function()
                while _G.QuestState.Fishing and _G.AutoFishing do
                    autoon()
                    task.wait(1)
                end
            end)
...
for _, quest in ipairs(Quests) do
    ...
    Tab8:Toggle({
        ...
        Callback = function(val)
            if val then
                -- Check Deep Sea completion only if quest is Element AND Deep Sea is actually incomplete
                if quest.Name == "Element" then
                    local deepSeaProgress = _G.getQuestProgress("Deep Sea")
                    if deepSeaProgress < 100 then
                        WindUI:Notify({
                            Title = "⚠️ Warning",
                            Content = "Complete Deep Sea quest first! Current: " .. deepSeaProgress .. "%",
                            Duration = 5,
                            Icon = "alert-triangle"
                        })
                        return
...
script:Button({
    ...
    Desc = "Other Scripts",
    Callback = function()
        loadstring(game:HttpGet('https://raw.githubusercontent.com/WhoIsGenn/VictoriaHub/refs/heads/main/Loader/Flygui.lua'))()
    end
})
