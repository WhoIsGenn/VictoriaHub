...
_G.CreateFloatingIcon = function()
    local old = _G.PlayerGui:FindFirstChild("CustomFloatingIcon_RockHub")
    if old then old:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "CustomFloatingIcon_RockHub"
    gui.DisplayOrder = 999
    gui.ResetOnSpawn = false

    local frame = Instance.new("Frame")
    frame.Size = UDim2.fromOffset(45, 45)
    frame.Position = UDim2.new(0, 60, 0.45, 0)
    ...
    gui.Parent = _G.PlayerGui
    return gui, frame
end

_G.SetupFloatingIcon = function(gui, frame)
    ...
    -- 🔥 DRAG PALING STABIL (TANPA InputChanged)
    _G.RunService.RenderStepped:Connect(function()
        if _G.dragging then
            local pos = _G.UserInputService:GetMouseLocation()
            local delta = pos - _G.dragStart
    ...
    if Window then
        Window:OnOpen(function()
            gui.Enabled = true -- icon tetap ada
        end)
