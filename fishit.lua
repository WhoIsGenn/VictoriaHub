...
_G.ReelSuper = 1.10
toggleState.completeDelays = 0.15
...
--========================================
-- AUTO PLACE TOTEM (UUID CAPTURE METHOD)
--========================================
...
local loopThread
local capturedUUIDs = {} -- Store captured UUIDs
...
task.spawn(function()
    ...
    if not netPath then return warn("[AutoTotem] Packages not found") end
    
    local indexPath = netPath:WaitForChild("_Index", 30)
    if not indexPath then return warn("[AutoTotem] _Index not found") end
    
    local sleitnickPath = indexPath:FindFirstChild("sleitnick_net@0.2.0")
    if not sleitnickPath then return warn("[AutoTotem] sleitnick_net not found") end
    
    local net = sleitnickPath:WaitForChild("net", 30)
...
-- Hook to capture UUIDs when manually spawning totems
local function setupUUIDCapture()
    print("[AutoTotem] Setting up UUID capture...")
    ...
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if self == SpawnTotemRemote and method == "FireServer" then
            ...
            if uuid and type(uuid) == "string" then
                -- Try to identify which totem this UUID belongs to
                -- We'll capture it and associate with the last spawned totem
                print(string.format("[AutoTotem] Captured UUID: %s", uuid))
                
                -- Store for later use
                table.insert(capturedUUIDs, 1, uuid) -- Insert at beginning
                
                -- Keep only last 10 UUIDs
                while #capturedUUIDs > 10 do
                    table.remove(capturedUUIDs)
                end
            end
        end
    ...
    print("[AutoTotem] UUID capture active!")
end

-- Initialize capture
if SpawnTotemRemote then
    setupUUIDCapture()
else
    task.wait(2)
    if SpawnTotemRemote then
...
-- Get most recent captured UUID
local function getLatestUUID()
    if #capturedUUIDs > 0 then
        return capturedUUIDs[1]
    end
    return nil
end

-- Find totem UUID (try GC first, then captured)
local function findTotemUUID(totemName)
    ...
    print("[AutoTotem] Searching for", totemName)
    
    -- First try: Search in GC
            ...
            if hasUUID and hasId then
                for _, totemId in ipairs(possibleIds) do
                    if v.Id == totemId then
                        print(string.format("[AutoTotem] Found in GC: UUID %s", v.UUID))
                        return v.UUID
                    end
                end
            end
    ...
    -- Second try: Use last captured UUID
    local latestUUID = getLatestUUID()
    if latestUUID then
        print(string.format("[AutoTotem] Using captured UUID: %s", latestUUID))
        return latestUUID
    end
...
local function placeTotem()
    ...
    local uuid = findTotemUUID(selectedTotem)
    
    if not uuid then
        WindUI:Notify({
            Title = "Auto Totem",
            Content = "No UUID found! Manually spawn " .. selectedTotem .. " once first!",
            Duration = 5
        })
        return false
    end
...
if not dataLoaded then
    ...
    PlaceTotem:Paragraph({
        Title = "Important",
        Content = "Manually spawn each totem once first to capture UUID!"
    })
    ...
    PlaceTotem:Button({
        Title = "Show Captured UUIDs",
        Desc = "View stored UUIDs",
        Callback = function()
            print("\n========== CAPTURED UUIDS ==========")
            if #capturedUUIDs == 0 then
                print("No UUIDs captured yet!")
                print("Manually spawn totems to capture their UUIDs.")
            else
                for i, uuid in ipairs(capturedUUIDs) do
                    print(string.format("[%d] %s", i, uuid))
                end
            end
            print("====================================\n")
        end
