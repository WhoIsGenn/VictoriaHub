...
animation = Tab3:Section({ 
    Title = "Skin Rod Animation",
...
animation:Dropdown({
    ...
    Callback = function(selected)
        ...
        print("Selected animation:", selected)
    end
})
...
animation:Button({
    ...
    Callback = function()
        if _G.SelectedAnim then
            _G.ActiveAnim = AnimationList[_G.SelectedAnim]
            if _G.ActiveAnim then
                print("Animation applied:", _G.SelectedAnim)
            else
                print("Animation disabled")
            end
        else
            print("Please select an animation first!")
        end
    end
})
...
-- Fungsi untuk mendapatkan nama totem dari ID
local function getTotemName(id)
    -- Mapping manual ID ke nama totem
    local totemNames = {
        [1] = "Luck Totem",
        [2] = "Mutation Totem", 
        [3] = "Shiny Totem"
    ...
    return totemNames[id] or "Totem ID: " .. tostring(id)
end

-- Fungsi untuk mendapatkan list totem unik
local function getUniqueTotemTypes()
    ...
    if inventory and inventory.Totems then
        for _, totem in pairs(inventory.Totems) do
            if not uniqueTotems[totem.Id] then
                uniqueTotems[totem.Id] = true
                local name = getTotemName(totem.Id)
                table.insert(totemNames, {Name = name, Id = totem.Id})
            end
        end
    end
...
local totemTypes = getUniqueTotemTypes()
local totemOptions = {}
for _, totem in pairs(totemTypes) do
    table.insert(totemOptions, totem.Name)
end
...
if #totemOptions > 0 then
    autoTotem:Dropdown({
        ...
        Callback = function(selected)
            for _, totem in pairs(totemTypes) do
                if totem.Name == selected then
                    _G.SelectedTotemId = totem.Id
...
autoTotem:Button({
    ...
    Callback = function()
        ...
        -- Cari totem dengan ID yang dipilih
        for _, totem in pairs(inventory.Totems) do
...
_G.TotemDelayMinutes = 60 -- Default 60 menit (1 jam)
...
autoTotem:Toggle({
    ...
    Callback = function(enabled)
        ...
        if enabled then
            local delaySeconds = _G.TotemDelayMinutes * 60
